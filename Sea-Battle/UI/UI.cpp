...
void UI::Render()
{
    ...
    const WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, _T("ImGui SeaBattle"), nullptr };
    ::RegisterClassEx(&wc);
    const HWND mainWindowHandle = ::CreateWindow(wc.lpszClassName, _T("ImGui SeaBattle"), WS_OVERLAPPEDWINDOW, 100, 100, 50, 50, NULL, NULL, wc.hInstance, NULL);

    // Hide console window
    ::ShowWindow(::GetConsoleWindow(), SW_HIDE);

    if (!CreateDeviceD3D(mainWindowHandle))
    {
        CleanupDeviceD3D();
    ...
    const HMONITOR monitor = MonitorFromWindow(mainWindowHandle, MONITOR_DEFAULTTONEAREST);
    MONITORINFO info = {};
